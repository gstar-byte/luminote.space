...
export async function categorizeThoughtFromImage(
  imageBase64: string,
  mimeType: string,
): Promise<CategorizeThoughtResult & { error?: string }> {
  const apiKey = await getGeminiKey();
  ...
  try {
    ...
    const textPrompt =
      ...
      '\n\nAnalyze the attached image. Extract any handwritten or printed text from the image and transcribe it. ' +
      'Then produce the same JSON object as for typed input: refinedContent must be the extracted and transcribed text; ' +
      'fill category, tags, isTodo, and reminder based on the extracted content when appropriate.';
    ...
    console.error('Failed to categorize from image:', error);
    return { refinedContent: '', error: 'API_ERROR' };
  }
}
