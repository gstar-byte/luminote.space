...
interface LandingPageProps {
  onLogin: (isRegistering?: boolean) => void;
}
...
export function LandingPage({ onLogin }: LandingPageProps) {
        ...
        window.google.accounts.id.initialize({
          ...
          callback: async (response: any) => {
            try {
              ...
              console.error('[GoogleSDK] Sign in with ID Token failed:', err);
            }
          },
          ux_mode: 'popup'
        });

        // 默认显示 One Tap 极速弹窗一键登录
        window.google.accounts.id.prompt();

        // 渲染官方的 Pill 胶囊形状原生登录按钮，完美契合页面设计
  ...
  const handleGoogleLogin = async () => {
    try {
      const { getAuth, getGoogleProvider, signInWithRedirect, signInWithPopup } = await import('../lib/supabaseAdapter');
      ...
      <section className="pt-40 pb-24 px-6 relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto text-center relative z-10">
          ...
          >
            ...
            {googleClientId ? (
              <div id="google-signin-btn-container" className="w-full sm:w-auto h-[52px] flex items-center justify-center" />
            ) : (
              <button onClick={handleGoogleLogin} className="w-full sm:w-auto px-6 py-4 bg-white/10 border border-white/20 text-white backdrop-blur-md rounded-full font-bold text-lg hover:bg-white/20 transition-all flex items-center justify-center gap-2">
                <svg className="w-5 h-5 bg-white rounded-full p-0.5" viewBox="0 0 24 24" aria-hidden="true"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
                Google
              </button>
            )}
          </motion.div>
      ...
      <footer className="py-12 border-t border-white/10 bg-black text-center relative z-10">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-2">
             <AppLogo className="w-[72px] h-[72px] md:w-[81px] md:h-[81px] shrink-0" />
